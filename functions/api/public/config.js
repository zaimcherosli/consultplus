...
export async function onRequestGet({ env }) {
  if (env.CONSULTPLUS_KV) {
    try {
      ...
      if (kvData && typeof kvData === 'object') {
        return jsonResponse({ success: true, config: { ...DEFAULT_SETTINGS, ...kvData } });
      }
    } catch (e) {}
  }
  ...
  if (env.DB) {
    try {
      ...
      const configMap = { ...DEFAULT_SETTINGS };
  ...
  return jsonResponse({ success: true, config: DEFAULT_SETTINGS });
}
